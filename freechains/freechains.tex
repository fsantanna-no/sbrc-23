...
notícias falsas.
Plataformas centralizadas adotam políticas de filto e anti abuso, mas exigem
confiança total dos seus usuários.
%
...
conteúdo ao mesmo tempo garantir consenso na rede.
%
Nós traçamos um paralelo com o Bitcoin da seguinte forma:
    novas postagens criam reputação (vs \emph{proof-of-work}),
...
the context of content sharing.
%
...
consensus mechanism applied to public forums.
In Section~\ref{sec.freechains}, we describe the concrete reputation rules we
implemented for public forums in \FC.
...
\footnote{
    Note that work from AI may be indistinguishable from humans to some extent,
...
reputation when deciding between branches in the forum DAG.
...
%
The removal rule for the remaining blocks is a direct consequence of
Merkle~DAGs~\cite{p2p.ipfs} used in permissionless protocols, which cannot
support graph modifications in order to remain tamper proof~\cite{p2p.bitcoin}.
...
forums of \FC to support content moderation and mitigate abuse.
Table~\ref{fig.rules} details the concrete rules we conceived, which are
discussed as follows through an example.
...
Section~\ref{sec.freechains.algo}.
