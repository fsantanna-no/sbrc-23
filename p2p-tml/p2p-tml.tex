...
%
Nesse trabalho, nós propomos um middleware para essas \emph{aplicações
peer-to-peer simétricas} nas quais instâncias descentralizadas podem efetuar
broadcasts de eventos assíncronos e exibirem comportamento idêntico.
%
...
%
Nós mostramos que o middleware suporta aplicações com mais de 20 pares em
topologias heterogêneas sob alto nível de churns.
...
Figure~\ref{fig.table} compares two selected works regarding these aspects.
%
...
solutions, but still viable in many scenarios, as we detail in
...
\hspace{-1cm}\textbf{(a) Event Deadlines:}
...
\hspace{-1cm}\textbf{(b) Time Synchronization:}
...
int main (void) {
    p2p_loop (
        ...
        cb_ini, cb_sim, cb_ren, cb_evt
            // callbacks: init/quit, simulation, rendering, events
    );
}
\end{verbatim}
}
...
\hspace{-1cm}\textbf{(a) Recurrence of time travels:}
...
\hspace{-1cm}\textbf{(b) Real-time pace of peers:}
...
\hspace{-1cm}\textbf{(c) Correctness of unstable peers:}
